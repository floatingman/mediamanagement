...
| NFS writes tear: files come back NUL-filled (`invalid load key, '\x00'` on reload), bursts of app-side I/O errors for ~1 min | **Failing array disk on the NAS** — Ratchet `sde` (WD80EFPX 8TB, array disk `bigmama8`) threw live read/write I/O errors from 2026-09-16 13:11; user-share I/O landing on it stalls/tears. Affected every NFS consumer, NOT a k8s regression | `ssh -p 2222 root@192.168.0.5 'dmesg -T | grep "I/O error"'`. NB (re-verified 2026-09-19; **corrects the 09-17 "parity-less" note below**): the Unraid md array is indeed empty (mdNumDisks=0), but `bigmama8` was a member of `/mnt/bigmama` — a **btrfs pool: Data RAID6 + Metadata RAID1C3 across 8 devices** — devid 8 is MISSING since the disk died and the pool is mounted `degraded`. Stripe reads that included it are reconstructed live from the 7 survivors; `btrfs device stats` shows corruption_errs on **all 7 remaining disks** (sdb 374, sde 284, sdf 136, sdc 123, sdg 66, sdh 32; SMART clean, 0 pending/reallocated) — reads that hit a bad extent fail csum → EIO/garbage through NFS = "sometimes corrupted downloads" (verified 09-19: one recent download reads clean, corruption is per-stripe). New Media writes **bypass the nvme cache** (cache free 686G < shareFloor 764.8G) and land directly on the degraded pool. Fix path: replace the disk → `btrfs replace missing /dev/sdX /mnt/bigmama` → full `btrfs scrub` (RAID6 self-repairs csum errors from parity once all 8 devices are present; scrub is useless while degraded). Also: Ratchet's `/var/log/syslog` has been 0 bytes since 09-16 (rsyslogd running but writing nothing) — use `dmesg`, not syslog, on the NAS. The 09-17 conclusion "the believed btrfs raid6 does not exist" was wrong (see MISTAKES.md 2026-09-19) — it checked only the md layer |
...
| VM NAS mount missing after reboot / `mount -a` does nothing for /mnt/* | Since 2026-09-19 the VM's NAS mounts are systemd units, NOT fstab: `mnt-{ratchetmedia,perceptormedia,perceptormediaexpansion,downloads,backups}.mount` from the `network_shares` role (`~/git/my-configuration`, persistent mode). fstab intentionally has zero active NAS lines | `systemctl status mnt-<name>.mount`; re-apply all: `cd ~/git/my-configuration && ~/.local/pipx/venvs/ansible/bin/ansible-playbook -i localhost play.yml --tags network_shares -e ansible_python_interpreter=/usr/bin/python3 --become` (passwordless sudo; `make configure TAGS=network_shares` prompts for a become password this VM doesn't need) |
| Who can write what on the Synology shares (model since 2026-09-21) | Deliberate scheme, not drift: NFS rules squash map-all-users-to-admin (uid 1024, whose primary group IS `users`/100), so every NFS client (pods PUID 1000, VM root/dnewman) executes as admin server-side; SSH/SMB `dnewman` is uid 1026, also group `users`. All shares normalized: group `users` everywhere, dirs `2775` (setgid — new files inherit the group), files `0664`, world loses write; writers run `UMASK=002` so new files stay group-writable | If dnewman can't write: check group (`ls -l` shows `:users`), the setgid bit on the parent dir, and the writer's UMASK (022 → 0644 files, read-only for the group). Re-normalize with the DSM Task Scheduler root task `perm-normalize` (chgrp -R users + find -type d chmod 2775 + find -type f chmod 0664; log /volume3/perm-normalize.log; proxbackup excluded — PBS semantics) |
...
| Torrent staging on the corrupt Ratchet pool (2026-09-19) | qb `complete_dir`-equivalent: qB writes `/downloads` = ratchetmedia `Downloads/torrents` (13 seeding eXo packs, ~1.9T); pool degraded per §3 | Copy then flip (in-container path never changes). TARGET: Synology `volume3/downloads/torrents` (the dedicated downloads share, alongside the usenet `nzb/` tree). COPY (direct NAS→NAS, Synology pulls): `ssh -p 2323 dnewman@192.168.0.6` then `setsid nohup /usr/bin/rsync -a --no-o --no-g --stats -e 'ssh -p 2222 -o BatchMode=yes -o UserKnownHostsFile=/tmp/syn-ratchet-kh' root@192.168.0.5:/mnt/user/Media/Downloads/torrents/ /volume3/downloads/torrents/ </dev/null >/tmp/torrent-pull-rsync.log 2>&1 &` — Synology SSH is port **2323** (uid 1026 dnewman, administrators; key-auth from the VM); perceptor pulls as root from Ratchet:2222 (perceptor's ~/.ssh/id_ed25519 authorized in Ratchet /root/.ssh/authorized_keys). MUST copy from the shfs UNION path `/mnt/user/...` (part of the tree lives on Ratchet's nvme cache tier). `--no-o --no-g`: chown/chgrp to source uids is impossible cross-NAS (expect cosmetic "failed to set permissions" lines on squashed-admin-era files + exit 23); after the copy, one `chmod -R a+rwX` AS dnewman over SSH on the target (fixes the 1026-owned majority; the 1024-owned remainder is already qB/NFS-writable). PUSH direction (anything→Synology rsync over SSH) does NOT work on DSM: sshd accepts the key then denies the session with a password prompt only for rsync-spawned connections — pull instead. Moving trees BETWEEN Synology shares: `mv` across shares is EXDEV (shares are separate btrfs subvolumes) → silent copy+delete fallback; if the ssh session dies mid-`mv` you get a split tree — use `rsync -a --remove-source-files` under setsid instead. FLIP when the pull log's `--stats` summary appears: edit `compose/downloaders.yaml` torrent volume `${RATCHET_MEDIA:-/mnt/ratchetmedia}/Downloads/torrents:/downloads` → `/mnt/downloads/torrents:/downloads`, `docker compose -f compose/downloaders.yaml up -d torrent`, verify `docker exec qbittorrent curl -s localhost:8181/api/v2/torrents/info | jq '.[].state'` all `stalledUP`, add a test torrent. k8s `deploy/torrent` (parked) re-pointed to `nfs-synology-downloads` subPath `torrents` 2026-09-19. Old ratchet tree kept until pool repair+scrub. NB: `nohup cmd &` from an agent/ssh shell dies with the shell — wrap in `setsid`; on the VM prefer `systemd-run` transient units (`systemctl reset-failed` before reusing a name); `pkill -f PATTERN` self-matches the wrapper shell |
| **FLIP DONE 2026-09-21:** copy verified byte-identical (18,926 files, 1.995TB), final `chmod -R a+rwX` as dnewman, compose volume now `/mnt/downloads/torrents:/downloads`, recreated via `docker compose up -d --no-deps torrent` FROM THE REPO ROOT — the running stack is compose project `mediamanagement` (root docker-compose.yaml `include:`s compose/*.yaml); `docker compose -f compose/downloaders.yaml` makes a FOREIGN project and fails with container-name/namespace conflicts. All 13 eXo packs `stalledUP` on the Synology backing, zero missing. Old ratchet tree kept until pool repair+scrub, then deletable |
...
| Completed downloads sometimes corrupt at rest / truncated imports (seen 2026-09-19) | Downloads were writing DIRECTLY to the degraded bigmama pool: Media share writes bypass the nvme cache (cache free 686G < shareFloor 764.8G) and the pool throws csum errors on all 7 survivors (§3) | `/downloads` backing swapped to the dedicated Synology `volume3/downloads` share (`nfs-synology-downloads` PV/PVC; NFS rule squash **map all users to admin** like the other shares; share root + `nzb/complete` tree pre-made 0777) in sabnzbd/radarr/sonarr/lidarr — same in-pod path, zero *arr config changes. Verified end-to-end 2026-09-19 (download → node-local PP → Synology → lidarr `downloadImported` → probe write on the final share). Interim backing was `nfs-perceptormediaexpansion` for a few hours the same day while the downloads share's NFS rule was being fixed — its rule broke MOUNTING when edited (mountd denied until set to map-all-users-to-admin). Torrent staging move: see qbittorrent table |
| Sabnzbd `PermissionError: [Errno 13]` creating a job dir under `/downloads/nzb/complete/<category>` on the Synology (seen 2026-09-20, `audio`) | The downloads share had a PRE-EXISTING tree from its SMB life: `mkdir -p` + parent-chmod at staging time no-ops over existing category dirs, and this one was owned by an orphaned uid (1000) with mode 0775 — neither dnewman (1026) nor the NFS-squash identity can chmod/write it | Probe every category dir FROM THE POD (`kubectl -n media exec deploy/sabnzbd -- sh -c 'touch /downloads/nzb/complete/<cat>/.probe'`). Fix without root: the parent is 777, so `mv <cat> <cat>.orphaned && mkdir <cat> && cp -a <cat>.orphaned/. <cat>/ && chmod 777 <cat>` (chmod AFTER the cp — `cp -a dir/.` re-applies the source dir's mode to the destination). Orphaned copies need a one-time DSM `sudo rm`. Root-owned 0777 dirs (nzb, nzb/complete) are fine — writable by everyone |
| *arrs log `Unable to connect to SABnzbd` in bursts while the queue is deep and DirectUnpack/unrar is running (seen 2026-09-22; 5 failures/min-poll) | **cgroup OOM killed the python process** (2Gi limit, working set rides 1.75Gi+ under load) — but the pod shows **0 restarts**: linuxserver's s6-overlay respawns the dead child inside the same container, masking the kill from kubectl. Six process restarts in 11 min = ~2 min API outage each | Count `kubectl -n media logs deploy/sabnzbd | grep -c 'SABnzbd.py-'` for restart banners (each banner pair = one silent respawn); confirm peak via Prometheus `container_memory_working_set_bytes{pod=~"sabnzbd.*"}`. Fix: memory limit 3Gi (manifest comment documents the incident) — node had 44% actual use |
...
| *arr: completed downloads sit in queue forever (`trackedDownloadState: importPending`, status ok, NO error in logs, imports never fire; seen on lidarr 2026-09-20) | **Wedged command blocking the serialized command queue**. Two layers: (1) a `started` command whose message never changes holds the whole queue — CDH and ManualImport sit `queued` behind it; `DELETE /api/v1/command/<id>` returns 409 (can't cancel started) → `kubectl -n media rollout restart deploy/<app>`. (2) the underlying wedge on lidarr was **memory**: 2Gi limit + .NET + 4.5G DB bulk-importing 15k tracks = GC death-spiral (88% CPU, RSS pinned at limit, zero progress, no logs — looks exactly like an I/O hang). Fixed with `limits: {memory: 4Gi}` | Diagnose: `GET /api/v1/command` → a `started` command with a static message = the blocker; if CPU is pegged and RSS is pinned at the limit, it's GC thrash, not NFS — raise the memory limit and restart. Verify post-restart that queued commands show ticking messages ("Reading file N/…"), then watch `history?eventType=3` for `downloadImported` events. Never hit lidarr with `/api/v1/album` unpaginated (full-library JSON OOMs it); use `/api/v1/wanted/missing` |
